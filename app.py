...
if mode == "📋 Top Produk (1 File)":
    ...
    if file:
        ...
        hasil = _format_top_produk(rekap, bulan, urut_param, persen)
        
        # Result header
        st.markdown(f"""
        <div class="result-header">
            <h3>✅ Berhasil! Top {persen}% Produk ({len(hasil)} SKU) — Diurutkan berdasarkan {urut}</h3>
            <p>📄 File: {file.name}</p>
        </div>
        """, unsafe_allow_html=True)
        
        # Display table
        _render_html_table(hasil)
...
# =========================================================
    ...
    if file_before and file_after:
        ...
        hasil = _format_before_after(rekap_before, rekap_after, bulan_before, bulan_after, urut_param_ba, persen)
        ...
        st.markdown(f"""
        <div class="result-header">
            <h3>✅ Berhasil! Perbandingan Harga — {kode_brand} Top {persen}% Produk ({len(hasil)} SKU)</h3>
            <p>📄 BEFORE: {file_before.name} → AFTER: {file_after.name}</p>
        </div>
        """, unsafe_allow_html=True)
        
        # Display table
        col_after_name = [c for c in hasil.columns if c.startswith("Rata2 Harga Jual")][1]
        _render_html_table(hasil, highlight_col=col_after_name)
